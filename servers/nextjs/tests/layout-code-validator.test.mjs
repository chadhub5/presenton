    ...
    plugins: [
      {
        ...
        setup(builder) {
          builder.onResolve({ filter: /^@\// }, (args) => ({
            path: path.join(projectRoot, args.path.slice(2)),
          }));
        },
      },
    ],
...
const Schema = z.object({
  title: z.string().default("Hello").max(40),
});
...
test("rejects missing Schema", async () => {
  const { LayoutCodeValidationError, validateLayoutCode } = await validatorPromise;
  const code = validCode.replace("const Schema = z.object", "const NotSchema = z.object");
...
test("rejects missing layout metadata", async () => {
  const { LayoutCodeValidationError, validateLayoutCode } = await validatorPromise;
  const code = validCode.replace('const layoutName = "Valid Layout";', "");
